...
function findSolution(target) {
    function find(current, history) { //does the actual recursing
        if (current == target) { //if it finds a solution
        ...
        } else { //if we are still below the target number
            return find(current + 5, `(${history} + 5)`) ||
            find(current * 3, `(${history} * 3)`); //the function tries both possible paths that start from the current number by calling itself twice, addition and multiplication
        }
    }
    return find(1, "1");
}
